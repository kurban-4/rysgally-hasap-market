        ...
        <div class="hero-stats">
            ...
            <div class="hero-stat">
                ...
                <div>
                    ...
                    <div class="hstat-value teal">{{ number_format($totalRevenue, 2) }} {{ __('app.currency_tmt') }}</div>
                </div>
            </div>
            <div class="hero-stat">
                    ...
                    <tr class="shift-row">
                        ...
                        <td>
                            <div class="revenue-cell">
                                ...
                                <span class="rev-currency">{{ __('app.currency_tmt') }}</span>
                            </div>
                        </td>
                        <td>
                            @if($shift->status === 'active')
                                <span class="status-badge status-active">
                                    ...
                                    {{ __('app.shift_status_active') }}
                                </span>
                            @else
                                <span class="status-badge status-closed">
                                    <i class="bi bi-check2 me-1"></i>
                                    {{ __('app.shift_status_closed') }}
                                </span>
                            @endif
                        </td>
                    </tr>
        ...
        <div class="mobile-shifts d-md-none">
            ...
            <div class="mobile-shift-card">
                <div class="msc-top">
                    ...
                    @if($shift->status === 'active')
                        <span class="status-badge status-active">
                            <span class="status-dot"></span> {{ __('app.shift_status_active') }}
                        </span>
                    @else
                        <span class="status-badge status-closed">
                            <i class="bi bi-check2 me-1"></i> {{ __('app.shift_status_closed') }}
                        </span>
                    @endif
                </div>
                <div class="msc-body">
                    <div class="msc-row">
                        <span class="msc-label">{{ __('app.shift_table_status') }}</span>
                        <span class="msc-val fw-bold" style="color: var(--teal);">
                            <i class="bi bi-display me-1"></i>
                            {{ $shift->till->name ?? '—' }}
                        </span>
                    </div>
                    <div class="msc-row">
                        <span class="msc-label">Касса</span>
                        <span class="msc-val fw-bold" style="color: var(--teal);">
                            <i class="bi bi-display me-1"></i>
                            {{ $shift->till->name ?? '—' }}
                        </span>
                    ...
                    <div class="msc-row">
                        <span class="msc-label">{{ __('app.shift_table_start') }}</span>
                        <span class="msc-val">{{ $shift->opened_at->format('d.m.Y H:i') }}</span>
                    </div>
                    <div class="msc-row">
                        <span class="msc-label">{{ __('app.shift_table_end') }}</span>
                        <span class="msc-val">
                            @if($shift->closed_at)
                    ...
                    <div class="msc-row">
                        <span class="msc-label">{{ __('app.shift_table_worked') }}</span>
                        @if($shift->closed_at)
                            @php
                                $totalMinutes = (int) $shift->opened_at->diffInMinutes($shift->closed_at);
                                $h = floor($totalMinutes / 60);
                    ...
                    <div class="msc-row">
                        <span class="msc-label">{{ __('app.shift_table_revenue') }}</span>
                        <span class="msc-val fw-bold teal">{{ number_format($shift->total_revenue, 2) }} {{ __('app.currency_tmt') }}</span>
                    </div>
                </div>
            </div>
            @empty
...
.till-pill {
    ...
    background: rgba(232,114,42,0.08); color: var(--teal);
